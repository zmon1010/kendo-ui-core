(function() {
    ...
    test("noRecords default template is rendered when option is enabled", function() {
    ...
    test("noRecords default template is not rendered when option is disabled", function() {
        ...
        equal(noRecords.length, 0);
    });

    test("noRecords is not rendered when the grid contains records", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
                scrollable: true,
                dataSource: [{foo: "bar"}],
                noRecords: true,
    ...
    test("noRecords is removed when continusly remove and add records", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
                scrollable: true,
                dataSource: [],
                noRecords: true,
        ...
        grid.dataSource.data([{foo:"bar"}]);
        equal(grid.table.parent().children('.k-grid-norecords').length, 0);
        grid.dataSource.data([]);
        equal(grid.table.parent().children('.k-grid-norecords').length, 1);
    });

    test("noRecords template is rendered when option is enabled", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
                scrollable: true,
                dataSource: [],
                noRecords: {
                ...
                columns: [{field:"foo", width:300}]}),
            noRecords = grid.table.parent().find(".foo");

        equal(noRecords.length, 1);
        equal(noRecords.text(), "bar");
    });

    test("noRecords template is not rendered when option is enabled and add records", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
                scrollable: true,
                dataSource: [],
                noRecords: {
        ...
        equal(grid.table.parent().find(".foo").length, 1);
        grid.dataSource.data([{foo:"bar"}]);
        equal(grid.table.parent().find(".foo").length, 0);
    });

    test("noRecords template is set using setOptions", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
            scrollable: true,
            dataSource: [],
            columns: [{field:"foo", width:300}]});
    ...
    test("noRecords template context is grid object", function() {
        var grid = new Grid($(table).wrap('<div style="width:200px"></div>'), {
            scrollable: true,
            dataSource: [],
            noRecords: {
                template: "<div class='foo'>#=this.options.pageable#</div>"
            },
            columns: [{field:"foo", width:300}]});

        var noRecords = grid.table.parent().find(".foo");
    ...
    test("autogenerated columns should be encoded", 2, function () {
        var grid = new Grid(div, { dataSource: [{ foo: "<span>foo</span>" }] });

        equal(grid.columns[0].encoded, true);
        equal(grid.tbody.find("td").html(), "&lt;span&gt;foo&lt;/span&gt;");
    });

    test("autogenerated columns should be encoded when dataSource transport is used", 2, function () {
        var grid = new Grid(div, { dataSource: { transport: { read: function (e) { e.success([{ foo: "<span>foo</span>" }]); } } } });

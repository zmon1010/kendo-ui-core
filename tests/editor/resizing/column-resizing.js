(function() {
    ...
    var HANDLE_WRAPPER_SELECTOR = DOT + "k-column-resize-handle-wrapper";
    ...
    test("hovering the border of the first cell should append resize handle", function() {
        ...
        equal($(columnResizing.options.rootElement).children(HANDLE_WRAPPER_SELECTOR).length, 1);
    });
    ...
    test("hovering the last cell should not create resize handle", function() {
        ...
        equal($(columnResizing.options.rootElement).children(HANDLE_WRAPPER_SELECTOR).length, 0);
    });
    ...
    test("resize handle should be stored as a reference", function() {
        ...
        equal($(columnResizing.options.rootElement).children(HANDLE_WRAPPER_SELECTOR)[0], columnResizing.resizeHandle[0]);
    });
    ...
    test("resize handle should be removed when leaving the cell border", function() {
        ...
        equal($(columnResizing.options.rootElement).find(HANDLE_WRAPPER_SELECTOR).length, 0);
    });
    ...
    test("resize handle should be removed when resizing a different column", function() {
        ...
        equal($(columnResizing.options.rootElement).find(HANDLE_WRAPPER_SELECTOR).length, 1);
    });
    ...
    test("resize hint marker should not be visible on column border hover", function() {
        ...
        equal($(columnResizing.options.rootElement).find(MARKER_SELECTOR).css("display"), "none");
    });
    ...
    test("should be visible on resize start", function() {
        ...
        equal($(columnResizing.options.rootElement).find(MARKER_SELECTOR).css("display"), "block");
    });
    ...
    module("editor column resizing keyboard", {
        setup: function() {
            wrapper = $("<div id='wrapper' contenteditable='true' />").appendTo(QUnit.fixture)[0];
            tableElement = $(TABLE_IN_PIXELS_WITH_COLUMNS_IN_PIXELS).appendTo(wrapper);
            columnResizing = new ColumnResizing(tableElement[0], {
            ...
            cell = $(columnResizing.element).find(FIRST_COLUMN);
            rootElement = columnResizing.options.rootElement;
        },
    ...
    test("should not be disabled when hovering a cell border", function() {
        triggerBorderHover(cell);
    ...
    editor_module("editor column resizing nested table initialization on mouseup", {
        beforeEach: function() {
            editor = $("#editor-fixture").data("kendoEditor");
            editor.columnResizing = null;
            tableElement = $(NESTED_TABLE_HTML).appendTo(editor.body)[0];
    ...
    test("should init new column resizing if resizing is in progress", function() {
        triggerEvent(tableElement, { type: MOUSE_ENTER });
        triggerEvent(nestedTable, { type: MOUSE_ENTER });
        editor.columnResizing.resizingInProgress = function() { return true; };
        ...
        ok(editor.columnResizing instanceof kendo.ui.editor.ColumnResizing);
        equal(editor.columnResizing.element, anotherTable);
    });

    test("should not init new column resizing if resizing is not in progress", function() {
        triggerEvent(tableElement, { type: MOUSE_ENTER });
        triggerEvent(nestedTable, { type: MOUSE_ENTER });
        editor.columnResizing.resizingInProgress = function() { return false; };
    ...
    test("should destroy current column resizing if resizing is in progress", function() {
        triggerEvent(tableElement, { type: MOUSE_ENTER });
        triggerEvent(nestedTable, { type: MOUSE_ENTER });
        editor.columnResizing.resizingInProgress = function() { return true; };
        var destroySpy = spy(editor.columnResizing, "destroy");
    ...
    test("should force resizing if resizing is in progress", function() {
        ...
        triggerEvent(nestedTable, { type: MOUSE_ENTER });
        editor.columnResizing.resizingInProgress = function() { return true; };
        editor.columnResizing.showResizeHandle(nestedTableCell, { buttons: 0, target: nestedTableCell });
        var resizeEndSpy = spy(editor.columnResizing._resizable.userEvents, "_end");

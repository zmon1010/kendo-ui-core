...
test("inline Editor toolbar does not show on focus when widget is readonly", function () {
    ...
    function assertEditorHandlers(actualHandlers, expectedHandlers) {
        equal(actualHandlers.length, expectedHandlers.length);

        for (var i = 0; i < actualHandlers.length; i++) {
            equal(actualHandlers[i].element[0], expectedHandlers[i].element);
            equal(actualHandlers[i].type, expectedHandlers[i].type);
        }
    }

    editor_module("editor registerHandler", {
        beforeEach: function() {
            editor = $("#editor-fixture").data("kendoEditor");
            editor._handlers = [];
            testElement = $("<div id='testElement'></div>").appendTo(QUnit.fixture)[0];
        },
    ...
    test("should add single event handler", function() {
        editor._registerHandler(testElement, "mousedown", $.noop);

        assertEvents(testElement, [{
            type: "mousedown",
            namespace: NS
        }]);
    });

    test("should cache single event handler", function() {
        editor._registerHandler(testElement, "mousedown", $.noop);

        assertEditorHandlers(editor._handlers, [{
            element: testElement,
            type: "mousedown" + DOT + NS
        }]);
    });

    test("should add multiple event handlers", function() {
        editor._registerHandler(testElement, "down", $.noop);
    ...
    test("should cache multiple event handlers", function() {
        editor._registerHandler(testElement, "down", $.noop);

        assertEditorHandlers(editor._handlers, [{
            element: testElement,
            type: "touchstart" + DOT + NS
        }, {
            element: testElement,
            type: "mousedown" + DOT + NS
        }]);
    ...
    editor_module("editor deregisterHandlers", {
        beforeEach: function() {
            editor = $("#editor-fixture").data("kendoEditor");
            editor._handlers = [];
            testElement = $("<div id='testElement'></div>").appendTo(QUnit.fixture)[0];
    ...
    test("should remove single event handler", function() {
        editor._registerHandler(testElement, "mousedown", $.noop);

        editor._deregisterHandlers();

        equal(jQueryEvents(testElement), undefined);
    });

    test("should remove cached single event handler", function() {
        editor._registerHandler(testElement, "mousedown", $.noop);

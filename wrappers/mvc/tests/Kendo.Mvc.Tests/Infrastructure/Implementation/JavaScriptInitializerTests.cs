...
namespace Kendo.Mvc.Infrastructure.Implementation.Tests
{
    public class JavaScriptInitializerTests
    {
        ...
        public void Should_serialize_char()
        {
            char foo = 'a';
            data["foo"] = foo;
            initializer.Serialize(data).ShouldEqual("{\"foo\":\"a\"}");
        }
        ...
        {
            data["foo"] = "foo";
            initializer.Initialize("#foo", "Foo", data).ShouldEqual("kendo.syncReady(function(){jQuery(\"#foo\").kendoFoo({\"foo\":\"foo\"});});");
        }
        ...
        {
            data["foo"] = "foo";
            initializer.Initialize("#foo.bar[0]", "Foo", data).ShouldEqual("kendo.syncReady(function(){jQuery(\"#foo\\\\.bar\\\\[0\\\\]\").kendoFoo({\"foo\":\"foo\"});});");
        }
        ...
        {
            data["foo"] = "foo";
            initializer.InitializeFor("#foo", "Foo", data).ShouldEqual("kendo.syncReady(function(){jQuery(\"#foo\").kendoFoo({\"foo\":\"foo\"});});");
        }
        ...
        {
            data["foo"] = "foo";
            initializer.InitializeFor("#foo.bar[0]", "Foo", data).ShouldEqual("kendo.syncReady(function(){jQuery(\"#foo.bar[0]\").kendoFoo({\"foo\":\"foo\"});});");
        }

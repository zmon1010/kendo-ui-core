...
namespace Kendo.Mvc.UI.Tests
{
    public class WidgetFactoryModelTests
    {
        ...
        private readonly IHtmlHelper<TestModel> htmlHelper;

        public WidgetFactoryModelTests()
        {
            var viewContext = TestHelper.CreateViewContext();
            var htmlHelperMoq = new Mock<IHtmlHelper<TestModel>>();

            htmlHelperMoq
                .Setup(s => s.ViewContext)
                .Returns(viewContext);

            var provider = new EmptyModelMetadataProvider();
            var viewData = new ViewDataDictionary<TestModel>(provider, new ModelStateDictionary());
            ...
            viewData.Model = new TestModel
            {
                ...
                TimeProperty = DateTime.Now.TimeOfDay,
                ComplexModel = new TestModel(),
                NullableShort = 5,
                NullableLong = 5,
            ...
            htmlHelper = htmlHelperMoq.Object;

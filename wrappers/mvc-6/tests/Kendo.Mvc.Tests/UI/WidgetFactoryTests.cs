...
namespace Kendo.Mvc.UI.Tests
{
    public class WidgetFactoryTests
    {
        ...
        private readonly WidgetFactory<TestModel> _factory;

        public WidgetFactoryTests()
        {
            var viewContext = TestHelper.CreateViewContext();
            var htmlHelper = new Mock<IHtmlHelper<TestModel>>();

            htmlHelper
                .Setup(s => s.ViewContext)
                .Returns(viewContext);

            _factory = new WidgetFactory<TestModel>(htmlHelper.Object);
        }
        ...
        //        .Name("foo").Deferred().Render();

        //    var renderScriptTags = false;
        //    var output = _factory.DeferredScripts(renderScriptTags);
